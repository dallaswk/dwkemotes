...
function EmoteCancel(force)
    if (not LocalPlayer.state.canCancel and GetConvar("onesync", "on") == "on") and not force then
        EmoteChatMessage(Translate('cancelblocked'))
        return
    end
...
--
-- Con esto activo, la animacion se reproduce solo en la parte superior del
-- cuerpo: el torso hace la emote y las piernas siguen respondiendo al mando,
-- asi que el jugador puede andar, correr y girar sin que la animacion se corte.
--
-- Tecnicamente es el flag 51 (AnimFlag.MOVING), que es la suma de:
--   1  bucle              32   el jugador conserva el control
--   2  quedarse en el ultimo fotograma
--   16 solo la parte superior del cuerpo
-- Es el mismo flag que ya llevan de fabrica las emotes marcadas con
-- `onFootFlag = AnimFlag.MOVING`; la novedad es poder aplicarselo a cualquiera
-- sobre la marcha.

--- ¿Se le puede aplicar el modo a esta animacion?
---@param emoteData table
---@param animOption? AnimationOptions
---@return boolean
local function walkLockApplies(emoteData, animOption)
    if not WalkLock then return false end
    -- Los escenarios los reproduce el juego entero, no aceptan flags.
    if emoteData and emoteData.scenario then return false end
    -- Una animacion colocada ancla al ped a un punto del mundo: dejarle andar
    -- la separaria del sitio en el que se coloco.
    if GetPlacementState() == PlacementState.IN_ANIMATION then return false end
    -- Las emotes que enganchan al ped a otro ped o a un prop dependen de que se
    -- quede quieto.
    if animOption and animOption.Attachto then return false end
    return true
end

--- Vuelve a lanzar la animacion en curso con los flags recalculados.
...
function ReplayCurrentAnimation()
    ...
    if not LoadAnim(currentEmote.dict) then return end

    local animOption = CurrentAnimOptions
    local flags = animOption and (animOption.Flag or animOption.onFootFlag) or 0
    if walkLockApplies(currentEmote, animOption) then
    ...
    PlayAnim(ped, currentEmote.dict, currentEmote.anim,
        ...
        flags, 0, false, false, false)
    RemoveAnimDict(currentEmote.dict)
end

--- @return boolean activo
function IsWalkLockEnabled()
    return WalkLock
end

--- Activa o desactiva el modo "caminar con la animacion".
---@param enabled boolean
---@param replay? boolean aplicar el cambio a la animacion que ya esta sonando
---@return boolean estado resultante
function SetWalkLock(enabled, replay)
    if not Config.WalkLockEnabled then return false end

    enabled = enabled and true or false
    if WalkLock == enabled then return WalkLock end
    WalkLock = enabled
...
--- @return boolean estado resultante
function ToggleWalkLock()
    local before = WalkLock
    local after = SetWalkLock(not before, true)
    if Config.WalkLockEnabled then
        SimpleNotify(Translate(after and 'walklockon' or 'walklockoff'))
    end
    return after
end

-- Estado de partida. Se guarda como cadena ("1"/"0") y no como entero a
...
CreateThread(function()
    ...
    local stored = Config.WalkLockPersistent and GetResourceKvpString(WalkLockKvp) or nil
    if stored == nil or stored == "" then
        WalkLock = Config.WalkLockDefault or false
    else
        WalkLock = stored == "1"
    end
end)
...
CreateExport("ToggleWalkLock", ToggleWalkLock)
...
function EmoteCommandStart(args)
    ...
    if (IsPedSwimming(PlayerPedId()) or IsPedSwimmingUnderWater(PlayerPedId())) and not Config.AllowInWater then
        TriggerEvent('chat:addMessage', {
            color = { 255, 0, 0 },
            multiline = true,
            args = { "dwkemotes", Translate('swimming') }
        })
        return
    end
...
function OnEmotePlay(name, textureVariation, emoteType)
    ...
    if not LocalPlayer.state.canEmote and GetConvar("onesync", "on") == "on" then
        EmoteChatMessage(Translate('emotesblocked'))
        return
    end
    ...
    if emoteData.scenario then
        if not inVehicle then
            -- Un escenario lo reproduce el juego de cuerpo entero y no acepta
            -- flags: aqui el modo no se puede aplicar y conviene decirlo, o
            -- parece que la funcion esta rota.
            if WalkLock then SimpleNotify(Translate('walklockscenario')) end
    ...
    if animOption then
        if animOption.PtfxAsset then
            ...
            PtfxPrompt = true
            runAnimationThread()
            TriggerServerEvent("dwkemotes:ptfx:sync", animOption.PtfxAsset, animOption.PtfxName, vector3(Ptfx1, Ptfx2, Ptfx3),
    ...
    -- "Caminar con la animacion": a pie, la animacion pasa a la parte superior
    -- del cuerpo para que las piernas sigan respondiendo al movimiento. Solo
    -- aplica fuera del vehiculo; dentro, el flag ya se decide arriba.
    if not inVehicle and walkLockApplies(emoteData, animOption) then
        flags = AnimFlag.MOVING
    ...
    if animOption and animOption.Prop then
        if Config.UseOldPropSpawning then
            ...
            SetPropStateBag({Emote = name, TextureVariation = textureVariation, emoteType = emoteType})
        end
