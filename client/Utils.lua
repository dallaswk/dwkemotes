...
RegisterNetEvent('dwkemotes:client:receivePermissions', function(manifest)
    local wasLoaded = permissions.loaded
    permissions = manifest
    permissions.loaded = true
    DebugPrint(string.format("[dwkemotes] Received permission manifest: mode=%s", permissions.mode))

    -- Initialize menu the first time permissions are received
    if not wasLoaded then
        InitMenu()
        DebugPrint("[dwkemotes] Menu initialized after receiving permissions")
    else
        -- Rebuild menu if permissions changed after initial load
        RebuildEmoteMenu()
        DebugPrint("[dwkemotes] Menu rebuilt due to permission update")
    end
end)
...
-- Registra un export bajo el nombre real del recurso y ademas bajo los alias
-- historicos, para que los scripts escritos contra rpemotes sigan funcionando
-- sin tocarlos (`exports.rpemotes:IsPlayerInAnim()` y equivalentes).
local EXPORT_ALIASES <const> = { 'rpemotes', 'rpemotes-reborn', 'dwkemotes' }

CreateExport = function(name, func)
    for _, alias in ipairs(EXPORT_ALIASES) do
...
--- Writes the prop state bag. Also mirrors it to the legacy key so third-party
--- resources written against rpemotes keep working. Only the primary key is listened to.
---@param value table
function SetPropStateBag(value)
    LocalPlayer.state:set(Config.StateBagKey, value, true)
    if Config.LegacyStateBagKey then
        LocalPlayer.state:set(Config.LegacyStateBagKey, value, true)
    end
end
...
AddStateBagChangeHandler(Config.StateBagKey, nil, function(bagName, key, value, reserved, replicated)
    ...
    if ServerProps[ped] and #ServerProps[ped] > 0 then
        ...
        DebugPrint("Deleted props for player. dwkemotes statebag update")
        ServerProps[ped] = nil
    end
