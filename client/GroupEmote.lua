...
RegisterNetEvent("dwkemotes:client:startGroupEmote", function(reqid, zone)
...
AddEventHandler("dwkemotes:client:autoCancel", function()
...
RegisterNetEvent("dwkemotes:client:requestGroupEmote", function(emotename, reqid, requestor, zone)
    ...
    while isRequestAnim do
        ...
        if timer <= 0 then
            isRequestAnim = false
            TriggerServerEvent("dwkemotes:server:refuseGroupEmote", reqid)
            ...
            if zone and zone.coords and zone.radius then
                if #(myCoords - zone.coords) <= zone.radius then
                    ...
                    SetTimeout((Config.GroupEmoteCountdownTime + 5) * 1000, function()
                        -- If state hasn't been cleared by dwkemotes:client:doGroupEmote, clear it now
