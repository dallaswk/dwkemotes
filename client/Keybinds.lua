CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/e', Translate('play_emote'), emoteArgs)
    TriggerEvent('chat:addSuggestion', '/emote', Translate('play_emote'), emoteArgs)
    TriggerEvent('chat:addSuggestion', '/emotemenu', Translate('open_menu_emote'))
    TriggerEvent('chat:addSuggestion', '/emotes', Translate('open_menu_emote'))
    TriggerEvent('chat:addSuggestion', '/dwkemotes', Translate('open_menu_emote'))
    TriggerEvent('chat:addSuggestion', '/emotecancel', Translate('cancel_emote'))

    if Config.WalkingStylesEnabled then
        TriggerEvent('chat:addSuggestion', '/walk', Translate('walkingstyles'),
            { { name = "style", help = Translate('resetdef') } })
    end
    if Config.ExpressionsEnabled then
        TriggerEvent('chat:addSuggestion', '/mood', Translate('moods'),
            { { name = "mood", help = Translate('resetdef') } })
    end
    if Config.WalkLockEnabled then
        TriggerEvent('chat:addSuggestion', '/emotewalk', Translate('walklockdesc'))
    end
end)
...
-- Caminar con la animacion: alterna que la emote se reproduzca solo en la
-- parte superior del cuerpo, dejando libres las piernas.
if Config.WalkLockEnabled then
    RegisterCommand('emotewalk', function() ToggleWalkLock() end, false)
    if Config.WalkLockKeybindEnabled then
        RegisterKeyMapping('emotewalk', Translate('register_walklock'), 'keyboard', Config.WalkLockKeybind)
    end
end

RegisterCommand('emotemenu', function() OpenEmoteMenu() end, false)
-- El original sugeria /emotes en el chat sin llegar a registrarlo.
RegisterCommand('emotes', function() OpenEmoteMenu() end, false)
RegisterCommand('dwkemotes', function() OpenEmoteMenu() end, false)
