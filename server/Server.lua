...
local function loadAndCacheEmotes()
    print("^3[dwkemotes] Loading and caching emote data...^0")
    ...
    if not animationFile then
        print("^1[dwkemotes] Failed to load AnimationList.lua^0")
        return false
    end

    local f, err = load(animationFile .. " return RP")
    if err then
        print("^1[dwkemotes] Error loading AnimationList.lua: " .. tostring(err) .. "^0")
        return false
    end

    local success, RP = pcall(f)
    if not success then
        print("^1[dwkemotes] Error parsing AnimationList.lua: " .. tostring(RP) .. "^0")
        return false
    end
    ...
    if customFile then
        ...
        if not customErr then
            ...
            if customSuccess and result then
                CustomDP = result
                print("^2[dwkemotes] Loaded custom emotes^0")
            end
        end
    end
    ...
    print(string.format("^2[dwkemotes] Cached %d emotes, %d shared, %d expressions, %d walks^0",
...
RegisterNetEvent("dwkemotes:server:requestEmote", function(target, emotename)
    local source = source
    ...
    TriggerClientEvent("dwkemotes:client:requestEmote", target, emotename, source)
end)

RegisterNetEvent("dwkemotes:server:confirmEmote", function(target, requestedemote, otheremote)
    local source = source
    ...
    TriggerClientEvent("dwkemotes:client:syncEmote", source, otheremote, target)
    TriggerClientEvent("dwkemotes:client:syncEmoteSource", target, requestedemote, source)
end)

RegisterNetEvent("dwkemotes:server:cancelEmote", function(target)
    TriggerClientEvent("dwkemotes:client:cancelEmote", target, source)
end)

RegisterNetEvent("dwkemotes:ptfx:sync", function(asset, name, offset, rot, bone, scale, color)
    assert(type(asset) == "string", "[dwkemotes] ptfx:sync: invalid asset for source: " .. tostring(source))
    assert(type(name) == "string", "[dwkemotes] ptfx:sync: invalid name for source: " .. tostring(source))
    assert(type(offset) == "vector3", "[dwkemotes] ptfx:sync: invalid offset for source: " .. tostring(source))
    assert(type(rot) == "vector3", "[dwkemotes] ptfx:sync: invalid rot for source: " .. tostring(source))
...
RegisterNetEvent("dwkemotes:server:syncHeading", function(heading)
    local state = Player(source).state
...
RegisterNetEvent('dwkemotes:server:requestPermissions', function()
    local source = source
    local permissions = buildPermissionManifest(source)
    TriggerClientEvent('dwkemotes:client:receivePermissions', source, permissions)
end)
