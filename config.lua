-- ============================================================================
--  dwkemotes - configuracion
--  Fork mejorado de rpemotes-reborn (NUI). Ver README.md para la lista completa.
-- ============================================================================

Config = {
    MenuLanguage = 'es', -- Idioma del menu (carpeta locales/)
    ...
    keybindKVP = "dwkemotes", -- Prefijo de las claves KVP locales (keybinds, favoritos, historial)
    MigrateLegacyKvp = true,  -- Importa una sola vez los datos guardados por rpemotes ("rpemotes_*")
    ...
    DisableControlsInMenu = false, -- Bloquea todos los controles del juego mientras el menu esta abierto
    DisableCombatInMenu = false,   -- Bloquea solo atacar, apuntar y cuerpo a cuerpo
    PreviewPed = true, -- Muestra una vista previa de la emote en un clon del ped junto al menu.
                       -- Cada jugador puede ajustar el retardo (o desactivarla) desde el panel de ajustes.
    ...
    -- Menu Styling
    -- El aspecto del menu (color de acento, columnas, escala, opacidad) lo elige
    -- cada jugador en el panel de ajustes; los valores de partida estan en Config.UI.
    MenuPosition = 'right', -- 'left' / 'right'. Lado en el que aparece el ped de vista previa.
    ...
    -- Bloquea la animacion en la parte superior del cuerpo para que las piernas
    -- sigan respondiendo al movimiento: el jugador puede andar, correr y girar
    -- mientras la animacion sigue sonando. Se activa desde el panel de ajustes
    -- del menu o con /emotewalk.
    -- No se aplica a los escenarios (el juego los reproduce de cuerpo entero),
    -- a las animaciones colocadas ni a las que enganchan al ped a otro ped.
    WalkLockEnabled = true,         -- Deja usar la funcion
    WalkLockDefault = false,        -- Estado de partida para quien nunca lo ha tocado
    WalkLockPersistent = true,      -- Recordar el estado entre sesiones
    WalkLockKeybindEnabled = false, -- Registrar una tecla propia. Ojo: puede chocar
                                    -- con otro recurso; el jugador puede recolocarla
                                    -- en Ajustes > Controles de FiveM.
    WalkLockKeybind = 'K',
    ...
    -- Developer Tools
    CheckForUpdates = false, -- Comprobacion de version contra Config.UpdateRepo (desactivada por defecto)
    ...
    RecentsEnabled = true,
    MaxRecents = 30,       -- Cuantas emotes recientes se recuerdan
    MostUsedMinCount = 3,  -- Usos minimos para aparecer en "Mas usados"
